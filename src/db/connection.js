...
async function createTables() {
    ...
    const createUsersTable = `
        CREATE TABLE IF NOT EXISTS users (
            ...
            subscription_last_reminder_days INTEGER,
            subscription_last_reminder_at DATETIME,
...
async function runMigrations() {
    ...
    if (!columnNames.includes('subscription_expires_at')) {
        migrations.push({
            ...
            sql: `ALTER TABLE users ADD COLUMN subscription_expires_at DATETIME`
        });
    }

    if (!columnNames.includes('subscription_last_reminder_days')) {
        migrations.push({
            name: 'add_subscription_last_reminder_days_column',
            sql: `ALTER TABLE users ADD COLUMN subscription_last_reminder_days INTEGER`
        });
    }

    if (!columnNames.includes('subscription_last_reminder_at')) {
        migrations.push({
            name: 'add_subscription_last_reminder_at_column',
            sql: `ALTER TABLE users ADD COLUMN subscription_last_reminder_at DATETIME`
        });
    }
    ...
    if (!columnNames.includes('payment_status')) {
        migrations.push({
            name: 'add_payment_status_column',
            sql: `ALTER TABLE users ADD COLUMN payment_status TEXT DEFAULT NULL`
        });
    }

    // Add payment_proof_file_id column if it doesn't exist
    if (!columnNames.includes('payment_proof_file_id')) {
        migrations.push({
            name: 'add_payment_proof_file_id_column',
            sql: `ALTER TABLE users ADD COLUMN payment_proof_file_id TEXT`
        });
    }

    // Add payment_requested_at column if it doesn't exist
    if (!columnNames.includes('payment_requested_at')) {
        migrations.push({
            name: 'add_payment_requested_at_column',
            sql: `ALTER TABLE users ADD COLUMN payment_requested_at DATETIME`
        });
    }

    // Add payment_submitted_at column if it doesn't exist
    if (!columnNames.includes('payment_submitted_at')) {
        migrations.push({
            name: 'add_payment_submitted_at_column',
            sql: `ALTER TABLE users ADD COLUMN payment_submitted_at DATETIME`
        });
    }
